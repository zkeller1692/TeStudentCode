...
public class Exercises {
    ...
    public String animalGroupName(String animalName) {
        ...
        Map<String, String> animalCollective = new HashMap<>();
        if (animalName == null) {
            return "unknown";
        }
        animalCollective.put("GIRAFFE", "Tower");
        animalCollective.put("RHINO", "Crash");
        animalCollective.put("ELEPHANT", "Heard");
        animalCollective.put("LION", "Pride");
        animalCollective.put("CROW", "Murder");
        animalCollective.put("FLAMINGO", "Pat");
        animalCollective.put("PIGEON", "Kit");
        animalCollective.put("DEER", "Heard");
        animalCollective.put("DOG", "Pack");
        animalCollective.put("CROCODILE", "Float");
        if (animalCollective.containsKey(animalName.toUpperCase())) {
            return animalCollective.get(animalName.toUpperCase());
        }
        return "unknown";
    }
    ...
    public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
        ...
        int peterNewMoney = peterMoney / 2;
        int paulNewMoney = paulMoney + peterNewMoney;

        if ((peterMoney > 0 && paulMoney < 1000) && (peterMoney % 2 != 0)) {
            peterPaul.put("Peter", peterNewMoney + 1);
            peterPaul.put("Paul", paulNewMoney);
            return peterPaul;
        } else if (peterMoney > 0 && paulMoney < 1000) {
            peterPaul.put("Peter", peterNewMoney);
            peterPaul.put("Paul", paulNewMoney);
            return peterPaul;
        }
        return peterPaul;
    ...
    public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
        int peterMoney = peterPaul.get("Peter");
        int paulMoney = peterPaul.get("Paul");
        int combinedSum = (peterMoney / 4) + (paulMoney / 4);
        int peterNewMon = peterMoney - (peterMoney / 4);
        int paulNewMon = paulMoney - (paulMoney / 4);

        if (peterMoney >= 5000 && paulMoney >= 10000) {
            peterPaul.put("Peter", peterNewMon);
            peterPaul.put("Paul", paulNewMon);
            peterPaul.put("PeterPaulPartnership", combinedSum);
            return peterPaul;
        }
        return peterPaul;
    }
    ...
    public Map<String, String> beginningAndEnding(String[] words) {
        Map<String, String> firstLastLetters = new HashMap<>();
        String first = "";
        String last = "";
        for (String word : words) {
            first = word.charAt(0) + "";
            last = word.charAt(word.length() - 1) + "";
            firstLastLetters.put(first, last);
        }
    ...
    public Map<String, Boolean> wordMultiple(String[] words) {
        ...
        for (String word : words) {
            ...
            map.put(word, stringCount.get(word) >= 2);
        }
        return map;
    }
    ...
    public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse,
        ...
        for (String name : mainWarehouse.keySet()) {
            if (remoteWarehouse.containsKey(name)) {
                totalInventory.put(name, (mainWarehouse.get(name) + remoteWarehouse.get(name)));
            } else {
                totalInventory.put(name, mainWarehouse.get(name));
            }
        }
        for (String name : remoteWarehouse.keySet()) {
            if (!(mainWarehouse.containsKey(name))) {
                totalInventory.put(name, remoteWarehouse.get(name));
            }
        }
