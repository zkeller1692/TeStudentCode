...
public class Exercises {
    ...
    public boolean hasTeen(int a, int b, int c) {
        boolean teenTestA = a >= 13 && a <= 19;
        boolean teenTestB = b >= 13 && b <= 19;
        boolean teenTestC = c >= 13 && c <= 19;

        if (teenTestA || teenTestB || teenTestC) {
            return true;
        }
        return false;
    }
    ...
    public boolean cigarParty(int cigars, boolean isWeekend) {
        boolean cigarsGood = cigars >= 40 && cigars <= 60;

        if (!isWeekend && cigarsGood) {
            return true;
        }
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
        if ((isBirthday && (mealAmount + 5) <= 10) || (!isBirthday && (mealAmount) <= 10)) {
            return "standard";
        } else if ((isBirthday && (mealAmount + 5 <= 15)) || (!isBirthday && (mealAmount) <= 15)) {
            return "special";
        } else {
            return "ginormous";
        }
